...
We can apply the basic sentiment tagger to the whole vector of comments. Remember that we need to use the text column without hyperlinks and emojis.
...
# ridgeline plot
library(ridgeline)
ridgeline(SentimentsLong$value,SentimentsLong$variable)
```
---
# 1) Basic Comment Sentiments
...
# Adding the Syuzhet Sentiments to our dataframe
...
# Histogram (mean words/comment = 10,6)
...
Because longer comments also contain more words, they have a higher likelihood of getting more extreme sentiment scores. Lets' look at one of the most negatively scored and one of the most positivly scored comments.

```{r, echo = TRUE, results = 'hide'}
# Very positivly scored comment
...
*Note*: We are using an absolute difference score here for ordering and order from larget to smallest difference

```{r, echo = TRUE}
# Illustrating scoring differences
...
From the examples in this data, we have seen multiple things:
 ...
 - Sentiment detection is not perfect. Bag-of-words approaches are often too simplistic
 - Even more sophisticated methods are misclassifying comments often
 - The choice of dictionary is highly important and can change the results substantially
 - Emoji seem to be even more complex for classifying sentiment

---
class: center, middle

# [Exercise](https://jobreu.github.io/youtube-workshop-gesis-2021/exercises/B2_Sentiment_Analysis_of_User_Comments_question.html) time `r ji("weight_lifting_man")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`
