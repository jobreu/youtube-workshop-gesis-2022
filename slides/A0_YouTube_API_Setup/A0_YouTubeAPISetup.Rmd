...
After that, you can click on "select project" on the top left of the screen.

![plot](Images/1.png)
---
# Creating a Project

In the pop-up window, click on "new project" to start a new project
...
![plot](Images/3.png)
---
# Activating YouTube Data API v3
...
On the next page, search for "youtube data api v3" and hit enter
...
On the next screen, click on "Create Credentials" on  the top right
...
Next, specify an arbitrary App name and enter your own new Gmail address as the user support email and developer contact email. "Save and continue" afterwards
![plot](Images/12.png)
---
# Creating Credentials
...
Use the uppermost checkbox to select all scopes on all pages of the table. For this workshop it's sufficient to only select YouTube scopes, but you could use these credentials for other APIs in the future as well. Afterwards, click on "update".
...
On the bottom of the page, click on "Done" to finalize your changes
...
In the sidebar panel opening from the right, enter your Gmail address as a test user and click on "save"

![plot](Images/19.png)
---
# Creating Credentials

Next, go back to the credentials page using the menu on the left side

![plot](Images/20.png)
---
# Creating Credentials

While we are here, we can also generate an API key that can be used for access to publicly available YouTube data using the API.
Click on the blue "Create Credentials" button on the top and select "API key".
...
Now click on the "Oauth 2.0 Client ID" name that you specified before
...
Congratulations! You now have your credentials! You can see your client ID and secret on the top right of the screen.
...
following commands in `R`. Please do *NOT* save an access token when prompted by R (see instructions in code comments)
...
Confirm your choices once more by clicking on "Allow". Your browser should now display the following message:
