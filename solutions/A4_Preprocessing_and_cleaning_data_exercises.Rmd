...
Load the comment data you scraped in the previous set of exercises into your `R` session and assign it to an object called `comments`. Get an overview of the contained variables. What do the variables describe? Why do we have missing data in some of them?
```

```{block, opts.label = "clues"}
To load the data, you can use the `readRDS()` function, to get an overview of the contained variables, you can simply use `colnames()`. To find out more about what the variables mean, you can have a look at the [*YouTube* data API documentation](https://developers.google.com/youtube/v3/docs) and search for the variable descriptions.
