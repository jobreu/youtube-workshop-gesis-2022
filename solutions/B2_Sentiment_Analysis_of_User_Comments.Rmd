...
Setup your `R` session and load your data (i.e., the parsed/processed comments) so that we can perform sentiment analysis. Assign the loaded data to a dataframe with the name `comments`. Load the necessary libraries to perform sentiment analysis.

```

```{block, opts.label = "clues"}
If your `R` version is < 4.0.0, you need to use the `options()` function to prevent `R` from interpreting your character variables as factor variables. If you are not sure how to use the `options()` function, you can always search it in the *RStudio* help panel and have a look at all the different options. After setting the options, you should load your preprocessed dataset with `readRDS()`. Don't forget to also load the required packages `syuzhet` and `sentimentr` using `library()`.
...
Zero values are given to comments containing no words from the used dictionary or containing multiple words with sentiment scores that cancel each other out **exactly**. Comments that only contain Emoji or a hyperlink will also have a score of zero as these are not contained in the dictionaries.
...
# Note: There are other and more sophisticated methods to count words in the packages stringr,stringi and tidytext. Before you use these, we recommend that you look at the rules
# they are using to define what is detected as a word and what isn't to make an informed decision of which method suits your needs best.
...
But how well a method performs on a given dataset always depends how well the data fits to the used dictionary and the complexity of the method. (Note that the fit between the sentiment scores for this video is better than for the Emoji Movie comments we saw on the slides.)
```
