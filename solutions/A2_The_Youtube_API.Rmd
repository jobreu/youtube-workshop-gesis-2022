...
```{block, box.title = "Exercise 4 (Bonus)", box.body = list(fill = "white"), box.icon = "fa-star"}
Construct and make an API call to get data with the following parameters:
...
- Request a _snippet_ from the method to get information about playlist items
...
You can check out the correct paramater names in the [documentation of the resource](https://developers.google.com/youtube/v3/docs/playlistItems/list)
...
You can execute your API call:
 ...
 - In your webbrowser by simply inserting the API call into the URL field
...
```{block, solution = TRUE, tidy= FALSE}
